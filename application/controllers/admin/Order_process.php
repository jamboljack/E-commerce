...
class Order_process extends CI_Controller {
	...
	public function index() {
		if($this->session->userdata('logged_in_furnindo')) {
			$data['listData'] 		= $this->order_model->select_all_process()->result();
			$this->template->display('admin/order_process_view', $data);
		} else {
	...
	public function updatedata() {
		...
		$this->session->set_flashdata('notification','Update Data Order Success.');
 		redirect(site_url('admin/order_process'));
	}
...
/* Location: ./application/controller/admin/Order_process.php */